...
export class Command<RequestT, ResponseT, ContextT> {
    constructor(
        ...
        protected readonly handler: (request: RequestT, session: Session<ContextT>) => Promise<ResponseT> | ResponseT,
    ) {}

    async invoke(session: Session<ContextT>, args: Element): Promise<{ result: ResultCode, responseId: number, response?: Element } | undefined> {

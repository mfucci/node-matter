...
export class PaseServer implements ProtocolHandler<MatterDevice> {
    ...
    private async handlePairingRequest(server: MatterDevice, messenger: PaseServerMessenger) {
        ...
        await server.createSecureSession(sessionId, undefined /* fabric */, UNDEFINED_NODE_ID, peerSessionId, Ke, new util.ByteArray(0), false, false, mrpParameters?.idleRetransTimeoutMs, mrpParameters?.activeRetransTimeoutMs);

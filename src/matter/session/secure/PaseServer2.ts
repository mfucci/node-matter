...
import { Spake2p2 } from "../../../crypto/Spake2p2";

const logger = Logger.get("PaseServer");

export class PaseServer2 implements ProtocolHandler<MatterDevice> {

    constructor(
        private readonly verificationValue: Buffer,
    ...
    private async handlePairingRequest(server: MatterDevice, messenger: PaseServerMessenger) {
        ...
        const spake2p = await Spake2p2.create(Crypto.hash([ SPAKE_CONTEXT, requestPayload, responsePayload ]), this.verificationValue);
